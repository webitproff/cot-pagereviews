<?php
/**
 * Page Reviews plugin - Edit mode
 * Description: Handles updating an existing review and editing via separate page
 * Filename: inc/pagereviews.edit.php
 * @package pagereviews
 * @version 1.0.1
 * @author webitprof
 * @copyright Copyright (c) 2025
 * @license BSD
 */

defined('COT_CODE') && defined('COT_PLUG') or die('Wrong URL');

require_once cot_langfile('pagereviews', 'plug');
require_once cot_incfile('pagereviews', 'plug');
require_once cot_incfile('forms');

list($usr['auth_read'], $usr['auth_write'], $usr['isadmin']) = cot_auth('plug', 'pagereviews', 'RWA');
cot_block($usr['auth_write']);

$t = new XTemplate(cot_tplfile(['pagereviews', 'edit'], 'plug'));

$pageid = cot_import('pageid', 'G', 'INT');
$itemid = cot_import('itemid', 'G', 'INT');
$redirect = cot_import('redirect', 'G', 'TXT');
$mode = cot_import('m', 'G', 'ALP');

// Получаем информацию о странице для редиректа
$page_info = $db->query("SELECT page_alias, page_cat, page_title FROM $db_pages WHERE page_id = ?", [$pageid])->fetch();
if (!$page_info) {
    cot_die_message(404, TRUE);
}
$default_url_params = !empty($page_info['page_alias']) ? ['c' => $page_info['page_cat'], 'al' => $page_info['page_alias']] : ['c' => $page_info['page_cat'], 'id' => $pageid];
$base_url = cot_url('page', $default_url_params, '', true);

// Проверяем отзыв
if (empty($itemid)) {
    cot_error('pagereviews_error_invalidreview');
    cot_redirect($base_url);
}

$sql = $db->query("SELECT * FROM $db_pagereviews WHERE item_id = ? AND item_pageid = ? LIMIT 1", [$itemid, $pageid]);
if ($sql->rowCount() == 0) {
    cot_error('pagereviews_error_invalidreview');
    cot_redirect($base_url);
}
$item = $sql->fetch();

cot_block($usr['isadmin'] || $usr['id'] == $item['item_userid']);

// Обработка формы (POST)
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    cot_shield_protect();

    $ritem['item_title'] = cot_import('rtitle', 'P', 'TXT');
    $ritem['item_text'] = cot_import('rtext', 'P', 'TXT');
    $ritem['item_score'] = (int)cot_import('rscore', 'P', 'INT');

    if ($usr['isadmin']) {
        $selected_userid = cot_import('ruserid', 'P', 'INT');
        if ($selected_userid > 0) {
            $user_exists = $db->query("SELECT user_id FROM $db_users WHERE user_id = ?", [$selected_userid])->fetch();
            if (!$user_exists) {
                cot_check(true, 'pagereviews_error_invaliduser');
            } else {
                $ritem['item_userid'] = $selected_userid;
            }
        }
        $rdate = cot_import_date('rdate', true);
        if ($rdate !== false && $rdate > 0) {
            $ritem['item_date'] = (int)$rdate;
        }
    }

    cot_check(empty($ritem['item_text']), 'pagereviews_error_emptytext');
    cot_check($ritem['item_score'] < 1 || $ritem['item_score'] > 5, 'pagereviews_error_emptyscore');
    cot_check(empty($ritem['item_title']), 'pagereviews_err_title', 'rtitle');

    if (!cot_error_found()) {
        $db->update($db_pagereviews, $ritem, "item_id = ? AND item_pageid = ?", [$itemid, $pageid]);

        // Отправка уведомлений
        $reviewer = $db->query("SELECT user_name FROM $db_users WHERE user_id = ?", [$item['item_userid']])->fetch();
        $reviewer_name = $reviewer['user_name'] ?: 'Неизвестный';
        $pageinfo = $db->query("SELECT page_title, page_alias, page_cat FROM $db_pages WHERE page_id = ?", [$item['item_pageid']])->fetch();
        $subject = $L['pagereviews_updated_review'] . ": " . htmlspecialchars($ritem['item_title']);
        $body = $L['pagereviews_updated_review_body'] . "\n\n";
        $body .= $L['pagereviews_title'] . ": " . htmlspecialchars($ritem['item_title']) . "\n";
        $body .= $L['pagereviews_text'] . ": " . htmlspecialchars($ritem['item_text']) . "\n";
        $body .= $L['pagereviews_score'] . ": {$ritem['item_score']}/5\n";
        $body .= $L['pagereviews_author'] . ": {$reviewer_name}\n";
        $body .= $L['pagereviews_page'] . ": {$pageinfo['page_title']} (" . $cfg['mainurl'] . '/' . cot_url('page', $default_url_params, '', true) . ")\n";
        $body .= $L['pagereviews_date'] . ": " . cot_date('datetime_full', $item['item_date']) . "\n";

        $superadmins = $db->query("SELECT user_email FROM $db_users WHERE user_maingrp = 5")->fetchAll();
        foreach ($superadmins as $admin) {
            if (!empty($admin['user_email'])) {
                cot_mail($admin['user_email'], $subject, $body);
            }
        }

        cot_message('pagereviews_success_updated');
        $redirect_url = empty($redirect) ? $base_url : base64_decode($redirect);
        cot_redirect($redirect_url);
    }
}

// Режим отображения формы редактирования (m=edit или прямой доступ)
$user_options = [];
if ($usr['isadmin']) {
    $users_sql = $db->query("SELECT user_id, user_name FROM $db_users ORDER BY user_name ASC");
    while ($u = $users_sql->fetch()) {
        $user_options[$u['user_id']] = htmlspecialchars($u['user_name']);
    }
}

// Приведение item_score к целому числу и обработка NULL
$score = isset($item['item_score']) ? (int)$item['item_score'] : 1; // Значение по умолчанию 1, если NULL

// Проверка языковых значений
$score_values = $L['pagereviews_score_values']; // [1, 2, 3, 4, 5]
$score_titles = $L['pagereviews_score_titles']; // ['1 звезда (Крайне отрицательно)', ...]

$date_value = !empty($item['item_date']) ? (int)$item['item_date'] : (int)$sys['now'];

$t->assign([
    'EDIT_FORM_ID' => $item['item_id'],
    'EDIT_FORM_SEND' => cot_url('plug', ['e' => 'pagereviews', 'm' => 'edit', 'itemid' => $itemid, 'pageid' => $pageid, 'redirect' => $redirect]),
    'EDIT_FORM_TEXT' => cot_textarea('rtext', $item['item_text'], 5, 50, ['class' => 'form-control', 'required' => 'required']),
    'EDIT_FORM_TITLE' => cot_inputbox('text', 'rtitle', $item['item_title'], ['class' => 'form-control', 'maxlength' => '255', 'required' => 'required']),
    'EDIT_FORM_SCORE' => cot_radiobox($score, 'rscore', $score_values, $score_titles, ['class' => 'form-check-input', 'required' => 'required']),
    'EDIT_FORM_USERID' => $usr['isadmin'] ? cot_selectbox($item['item_userid'], 'ruserid', array_keys($user_options), array_values($user_options), true, ['class' => 'form-control']) : '',
    'EDIT_FORM_DATE' => $usr['isadmin'] ? cot_selectbox_date($date_value, 'long', 'rdate', date('Y', $sys['now']), date('Y', $sys['now']) - 5, false, '', ['class' => 'form-control d-inline-block w-auto']) : '',
    'EDIT_FORM_BACK_URL' => $redirect ? base64_decode($redirect) : $base_url,
]);

cot_display_messages($t);
?>