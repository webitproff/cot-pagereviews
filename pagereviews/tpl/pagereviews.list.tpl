...
<div class="border-bottom border-secondary py-3 px-3">
  <nav aria-label="breadcrumb">
    <div class="ps-container-breadcrumb">
      <ol class="breadcrumb d-flex mb-0">{PHP.L.pagereviews_title}</ol>
    </div>
  </nav>
</div>
<!-- BEGIN: MAIN -->
<div class="container-xl py-4">
  ...
  <h2 class="h5 mb-4">{PHP.L.pagereviews_all_reviews_page_desc}</h2> {FILE "{PHP.cfg.themes_dir}/{PHP.cfg.defaulttheme}/warnings.tpl"}
</div>
<div class="container-xl py-4">
  <div class="p-3 rounded-2" style="border: 5px var(--bs-primary-border-subtle) solid">
    <form action="{SEARCH_ACTION_URL}" method="get" class="mb-4">
      ...
      <div class="row g-3 align-items-end">
        <div class="col-md-3">
          <label for="sq" class="form-label">{PHP.L.pagereviews_search_reviews}</label> {SEARCH_SQ}
        </div>
        <div class="col-md-3">
          <label for="c" class="form-label">{PHP.L.pagereviews_category}</label> {SEARCH_CAT}
        </div>
        <div class="col-md-3">
          <label for="user" class="form-label">{PHP.L.pagereviews_user}</label> {SEARCH_USER}
        </div>
        <div class="col-md-3">
          <label for="sort" class="form-label">{PHP.L.pagereviews_sort}</label> {SEARCH_SORTER}
        </div>
      ...
      <div class="row">
        <div class="col-12 col-sm-3 d-none d-sm-block">
          ...
          <a class="btn btn-warning w-100 w-md-auto" href="{PHP|cot_url('pagereviews', 'm=claim')}">
            <i class="fa-solid fa-exclamation-triangle"></i> {PHP.L.pagereviews_complaints} </a>
          <!-- ENDIF -->
        </div>
          ...
          <div class="row g-3 justify-content-md-end justify-content-center">
            <div class="col-md-6 col-12 text-center">
              <button type="submit" class="btn btn-primary w-100">
                <i class="fa-solid fa-search"></i> {PHP.L.2wd_StartSearch} </button>
            </div>
            <div class="col-md-6 col-12 text-center">
          ...
          <div class="row g-3">
            <div class="col-12 col-md-8">
              <h5 class="mb-0 fs-6 fw-semibold text-primary-emphasis">
                ...
                <span class="ms-2">№{REVIEW_ROW_ID} <a href="{REVIEW_ROW_URL}">{REVIEW_ROW_TITLE}</a>
                </span>
              </h5>
              <p class="mb-0">{REVIEW_ROW_TEXT|cot_string_truncate($this, 150)} ...</p>
              <p class="mb-1 text-body-secondary text-truncate">{PHP.L.pagereviews_page}: <a href="{REVIEW_ROW_PAGE_URL}">{REVIEW_ROW_PAGE_TITLE}</a>
              </p>
            </div>
            ...
            <div class="row g-3 justify-content-md-start justify-content-center">
              ...
              <div class="col-md-auto col-12 text-center">
                <a href="{REVIEW_ROW_DELETE_URL}" class="btn btn-danger btn-sm delete-review" data-bs-toggle="modal" data-bs-target="#deleteConfirmModal" data-delete-url="{REVIEW_ROW_DELETE_URL}">
                  <i class="fa-solid fa-trash"></i> {PHP.L.Delete} </a>
              </div>
              <!-- ENDIF -->
              <!-- IF {REVIEW_ROW_EDIT_URL} -->
              <div class="col-md-auto col-12 text-center">
                <a href="{REVIEW_ROW_EDIT_URL}" class="btn btn-primary btn-sm">
                  <i class="fa-solid fa-edit"></i> {PHP.L.Edit} </a>
              </div>
              <!-- ENDIF -->
              <!-- IF {REVIEW_ROW_COMPLAINT_URL} -->
              <div class="col-md-auto col-12 text-center">
                <a href="{REVIEW_ROW_COMPLAINT_URL}" class="btn btn-warning btn-sm">
                  <i class="fa-solid fa-exclamation-triangle"></i> {PHP.L.pagereviews_submit_complaint} </a>
              </div>
              <!-- ENDIF -->
            </div>
          </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
          <i class="fa-solid fa-xmark"></i> {PHP.L.Close} </button>
      </div>
    </div>
  </div>
</div>
